...
class LightRAG:
    ...
    async def aquery_llm(
        ...
        try:
            ...
            ccreslut = query_result.raw_data["data"]
            output_dir = "../lightrag-results"

            for name, records in ccreslut.items():
                if not records:  # 空表就跳過
                    print(f"  Skip {name} because it is empty")
                    continue
                df = pd.DataFrame(records)
                df.insert(0, "query", query.strip())
                filename = os.path.join(output_dir, f"lightrag-{param.mode}-search-{name}.csv")

                # 判斷是否要 append 還是 create
                if os.path.exists(filename):
                    df.to_csv(filename, mode="a", index=False, encoding="utf-8", header=False)
                    print(f"  Appended to: {filename}")
                else:
                    df.to_csv(filename, index=False, encoding="utf-8")
                    print(f"  Created: {filename}")

            print(f"Finished saving results for query: {query.strip()}")
