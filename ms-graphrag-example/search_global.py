...
class GlobalSearch(BaseSearch[GlobalContextBuilder]):
    ...
    async def stream_search(
        ...
        for name, df in context_result.context_records.items():
            if df.empty:
                print(f"  Skip {name} because DataFrame is empty")
                continue
            filename = f"ms-graphrag-results/global-search-{name}.csv"
            df_copy = df.copy()
            df_copy.insert(0, "query", query)
            if os.path.exists(filename):
                df_copy.to_csv(filename, mode="a", index=False, encoding="utf-8", header=False)
                print(f"  Appended to: {filename}")
            else:
                df_copy.to_csv(filename, index=False, encoding="utf-8")
                print(f"  Created: {filename}")
        print(f"Finished saving results for query: {query}")
